...
const refreshCredentialError = (credential) => {
...
export const findRefreshSessionByTokenHash = (refreshCredential) => {
  if (!refreshCredential?.ok) {
    return refreshCredentialError(refreshCredential);
  }

  const tokenId = String(refreshCredential.tokenId || "");
  const refreshTokenRaw = String(refreshCredential.raw || "");
  if (!tokenId || !refreshTokenRaw) {
    return refreshSessionError(
      401,
      "AUTH_REFRESH_INVALID",
      "刷新令牌无效，请重新登录",
