...
const AUTHUSER_TIMEOUT_MS = 8_000;
const AUTHUSER_RETRY_COUNT = 2;
...
const SUPPORTED_TASK_IDS = new Set([
  "daily",
  "hangup",
  "bottle",
  "tower",
...
const EMPTY_PRIORITY = Number.MAX_SAFE_INTEGER;
let tokenExecutionChain = Promise.resolve();
let tokenExecutionQueueLength = 0;
const MAX_TOKEN_EXECUTION_QUEUE_LENGTH = 300;
const MAX_AUTO_TASKS_PER_USER_PER_TICK = 20;
const WS_CONNECT_TIMEOUT_MS = 12_000;
const WS_CONNECT_RETRY_COUNT = 2;
const WS_CONNECT_RETRY_BASE_DELAY_MS = 500;
const WS_CIRCUIT_OPEN_FAIL_COUNT = 3;
const WS_CIRCUIT_OPEN_MS = 60_000;
const TASK_CONTROL_CUSTOM_WS_URL_ENABLED = ["1", "true", "on", "yes"].includes(
  String(process.env.TASK_CONTROL_CUSTOM_WS_URL_ENABLED || "").trim().toLowerCase(),
);
const TASK_CONTROL_CUSTOM_WS_URL_ALLOWLIST = String(
...
const formatExecutionErrorMessage = (error) => {
...
const parseTaskRows = (payloadJson) => parseTaskControlRowsFromPayload(payloadJson);
...
const parseTokenString = (tokenText) => {
...
const maskTokenIdForLog = (tokenId) => {
